...
export interface IDistrictesProps {
    data: IDistrictOutside[] | undefined | null;
    placeholder?: string;
    messageUndefined?: string;
    initData?: string;
    label?: string;
    name: 'province' | 'district' | 'ward';
    onValue?: (value: IDistrictOutside | undefined) => void;
    onValidate?: (validateFuc: () => boolean) => void;
}

export default function Districtes({ data, placeholder, messageUndefined, initData, name, label, onValue, onValidate }: IDistrictesProps) {
    ...
    const renderData = useCallback(() => {
        ...
        return curData.map((item) => {
            return (
                <li
                    onClick={() => handleClickItem(item.DistrictName)}
