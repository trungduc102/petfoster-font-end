...
import { AddRecipient, TextArea, TextField, WrapperAnimation } from '../..';
...
export default function NotificationDialog({ idOpen, open, setOpen }: INotificationDialogProps) {
    // redux
    ...
    const [recipients, setRecipients] = useState<IUserFirebase[]>([]);
    ...
    useEffect(() => {
        ...
        setPhoto(curPhoto);
    }, [typeNotification, imagesDefault]);
    ...
    const handleCloseRecipient = (e: MouseEvent<HTMLDivElement>, data: IUserFirebase) => {
        e.stopPropagation();

        const newData = recipients.filter((item) => item.username !== data.username);
        ...
        if (photo?.file) {
            try {
                ...
                await firebaseService.addNotification({
                    ...
                    read: [],
                    target: recipients.map((item) => item.username),
                    title: form.title,
                    type: typeNotification,
                    options: {
                        ...positionText,
    ...
    return (
        ...
        >
            <div className="scroll">
                    ...
                    <div className="flex items-center gap-5">
                        ...
                        {idOpen && (
                            <div className="flex items-center gap-4 flex-wrap">
                                <WrapperAnimation hover={{}} className={classNames('rounded-2xl border  text-sm py-2 px-4 cursor-pointer font-semibold min-w-[80px] text-center')}>
                                    {'khangkhang'}
                                </WrapperAnimation>
                            </div>
                        )}
                            ...
                            <div className="flex flex-1 items-center gap-4 w-full flex-wrap">
                                {recipients.map((item, index) => {
                                    return (
                                        ...
                                        >
                                            <p className="flex-1 truncate">{item.username}</p>
                                ...
                                <AddRecipient
                                    onUser={(user) => {
                                        const isExistUser = recipients.some((item) => item.username === user.username);
                ...
                {isViewerOpen && imagesDefault && (
                    <ImageViewer
                        src={idOpen ? [imagesDefault[0].photourl] : imagesDefault.map((item) => item.photourl)}
